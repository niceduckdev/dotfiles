...
# Function to extract file ID and construct the download URL
def get_download_url(drive_url):
    match = re.search(r'/file/d/([a-zA-Z0-9_-]+)', drive_url)
    if match:
        file_id = match.group(1)
        return f'https://drive.usercontent.google.com/uc?id={file_id}'
    return None
...
for index, row in df.iterrows():
    drive_url = row['url']
    download_url = get_download_url(drive_url)
    ...
    try:
        ...
        if 'image' not in content_type:
            print(f"URL {download_url} does not contain an image.")
            continue
        ...
        print(f"Failed to identify image from {download_url}")
    except Exception as e:
        print(f"An error occurred with URL {download_url}: {e}")
